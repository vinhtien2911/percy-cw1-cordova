...
function getCurrentLocation() {
    if(navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(function(position) {
            ...
            gps = `https://maps.google.com/?q=${position.coords.latitude},${position.coords.longitude}`
        })
    } else {
...
$(document).on('click', '#page-list #btn-resetall', showList);
...
function confirmTrip(e) {
    ...
    $('#page-create #frm-confirm #phonenumber').text($('#page-create #frm-register #phonenumber').val());
    $('#page-create #frm-confirm #hotel').text($('#page-create #frm-register #hotel').val());
...
function registerTrip(e) {
    ...
    let phone = $('#page-create #frm-register #phonenumber').val();
    let hotel = $('#page-create #frm-register #hotel').val();

    db.transaction(function (tx) {
        let query = `INSERT INTO Trip (Name, Destination, Description, Risk, Phone, Hotel, Gps, Date) VALUES (?, ?, ?, ?, ?, ?, ?, julianday('${date}'))`;
        tx.executeSql(query, [name, destination, description, risk, phone, hotel, gps], transactionSuccess, transactionError);
    ...
    db.transaction(function (tx) {
        ...
        function transactionSuccess(tx, result) {
            if (result.rows[0] != null) {
                ...
                $('#page-detail #detail #phonenumber').text(result.rows[0].Phone);
                $('#page-detail #detail #hotel').text(result.rows[0].Hotel);
                $('#page-detail #detail #gps').attr('href', result.rows[0].Gps);
    ...
    db.transaction(function (tx) {
        ...
        function transactionSuccess(tx, result) {
            if (result.rows[0] != null) {
                ...
                $(`#page-detail #frm-update #phonenumber`).val(result.rows[0].Phone);
                $(`#page-detail #frm-update #hotel`).val(result.rows[0].Hotel);
...
function updateTrip(e) {
    ...
    let phone = $('#page-detail #frm-update #phonenumber').val();
    let hotel = $('#page-detail #frm-update #hotel').val();

    db.transaction(function (tx) {
        let query = `UPDATE Trip SET Name = ?, Destination = ?, Description = ?, Risk = ?, Phone = ?, Hotel = ?, Date = julianday('${date}') WHERE Id = ?`;

        tx.executeSql(query, [name, destination, description, risk, phone, hotel, id], transactionSuccess, transactionError);
